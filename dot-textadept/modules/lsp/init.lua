...
function Server:request(method, params)
  ...
  repeat
    message = self:read()
    if not message then -- nil if the lsp process crashed or was killed
      break -- ...in which case the response would never ever come in
    end
  ...
  return message and message.result ~= json.null and message.result or nil
end
